...
| Firmware on actual hardware | Reproducible build, protocol/storage tests, real microphone/radio/charge/haptic/boot/OTA and fault tests | Monitored PDM driver, bounded audio FIFO, recorder, Opus/archive, owned NAND journal, SPI control-pair adapter and durable release controller implemented. Host checks: 20 driver, 10 adapter, 10 recorder, 25 SPI, 19 journal and 13 storage groups (745 cases), plus 33 receiver tests. Real C/Opus → committed Python receiver/outbox → C release/reuse preserves another recording on modeled NAND. A separate [DK bench application](../../firmware/a04/bench/README.md) connects actual peripheral drivers with named wiring, UART capture/export through the bounded cursor, 31 host tests and five orchestration groups (45 cases); its ARM build is separate from the synthetic probe. No physical execution, production audio erase or complete A04 firmware yet |
