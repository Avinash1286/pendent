...
This directory contains an actual libopus encoder, a bounded PCM-to-packet capture wrapper, a portable revision-3 archive writer, a packed recoverable NAND journal, a W25N01GV command core/Zephyr SPI adapter, host failure/round-trip tests, and an isolated Cortex-M4 build. A matching Python reader and durable receiver import real C-encoded recordings. It is **not the complete A04 wearable firmware**. It does not alter the published A03 firmware or claim working physical microphones, NAND storage, BLE, signed updates or measured battery life.
...
The current integrated build passes **20 SPI command/integration groups, 11 journal groups and 33 Python archive/receiver tests**. Four real-Opus journal recovery cases also pass export, SQLite restart/replay and independent FFmpeg decode. These are host tests and simulated storage faults; none establishes physical flash or microphone operation.
...
The actual nRF52840 cross-build, including the journal, SHA256/CRC and retained SPI adapter, uses **194,356 bytes of flash (18.54%)** and **148,864 bytes of RAM (56.79%)**. This includes a 32,768-byte encoder arena, a 49,216-byte guarded thread-stack object (48 KiB usable reservation plus alignment/guard overhead), a 1,992-byte capture context, a 1,496-byte archive writer, a 35,880-byte journal, a 4,464-byte Zephyr NAND adapter, 16,384 bytes of synthetic RAM NAND pages and a 640-byte input buffer. These are linked resource reservations for the isolated synthetic probe, not the complete product budget. [arm-resources.json](verification/arm-resources.json) records ELF symbols, source/config hashes, artifact identities and compiler stack reports; [arm-build.txt](verification/arm-build.txt) retains the successful linker output. No Cortex-M4 runtime timings or stack high-water measurements are claimed. The existing A03 release hash/source checker also passes unchanged.
