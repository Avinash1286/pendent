...
const routingReport=fs.existsSync('output/routed-validation.json')?JSON.parse(fs.readFileSync('output/routed-validation.json','utf8')):null;
const routed=!!routingReport?.counts?.pcb_trace&&!routingReport?.counts?.pcb_autorouting_error;
const source=path.resolve(`output/aura-${routed?'routed':'placement'}.circuit.json`);
for(const [format,filename] of [['kicad_pcb',`aura-${routed?'routed':'placement'}.kicad_pcb`],['kicad_sch','aura.kicad_sch'],['specctra-dsn','aura.dsn']]){
 ...
 if(format==='kicad_pcb'){
  ...
  fs.writeFileSync(target,restoreMicLand(data));
 }
}
