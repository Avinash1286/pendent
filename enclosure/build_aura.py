...
# Back pogo contacts in an isolated flush polymer bed.
pogo_cut=cube('Charging bed opening',(0,-14.4,-5.65),(10.0,3.1,3),dark,1.2,False)
boolean(rear,pogo_cut)
cube('Charging_Contact_Bed',(0,-14.4,-5.70),(9.8,2.9,.50),dark,1.15)
for i,x in enumerate([-3,-1,1,3]):cyl(f'Charging_Contact_{i+1}',(x,-14.4,-5.983),.51,.10,gold)
...
for y in [-18.4,18.4]:
    head=cyl('Case_Screw',(0,y,-5.53),.88,.26,buttonmat)
...
if '--no-render' not in sys.argv:
    ...
    poses={ob.name:ob.location.copy() for ob in product}
    ...
    for ob in product:
        if any(ob.name.startswith(p) for p in front_parts):ob.location.z+=20*MM
        elif any(ob.name.startswith(p) for p in rear_parts):ob.location.z-=22*MM
        elif ob.name.startswith(('Battery_Envelope','Battery_Insulator','Battery_Label')):ob.location.z-=12*MM
        elif ob in cords:ob.hide_render=True
        elif ob.name.startswith('Acoustic_Gasket'):ob.hide_render=True
    ...
    for ob in product:ob.location=poses[ob.name];ob.hide_render=False
    floor.hide_render=True
    camera((62,-45,112),(0,10,0),77)
