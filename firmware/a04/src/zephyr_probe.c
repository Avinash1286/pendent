...
static uint32_t sink_packets, sink_bytes;
static int16_t input[AURA_OPUS_MAX_SAMPLES];

static int probe_sink(void *user, const struct aura_opus_packet *packet)
{
    ARG_UNUSED(user);
    ...
    if (packet->sequence != sink_packets) return -1;
    ++sink_packets;
    sink_bytes += packet->bytes;
    return 0;
}
...
static void probe(void *one, void *two, void *three)
{
    ...
    int status = aura_opus_init(&capture, encoder_state.bytes, sizeof(encoder_state.bytes),
                                20, probe_sink, NULL);
    printk("A04 EXPERIMENTAL codec=%s state=%u reserved=%u context=%u init=%d\n",
           opus_get_version_string(), (unsigned)aura_opus_state_bytes(),
           (unsigned)sizeof(encoder_state.bytes), (unsigned)sizeof(capture), status);
    ...
    printk("A04 PROBE ONLY result=%d packets=%u bytes=%u source=%u preskip=%u tail=%u "
           "worst_encode_us=%u late_frames=%u stack_free=%u stack_check=%d\n",
           status, sink_packets, sink_bytes, (unsigned)seal.source_samples,
