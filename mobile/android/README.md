...
The consumer Activity remains the phone-side file-import foundation for A04. It does **not** yet connect to a pendant over BLE, enroll a device, download recordings over the air, transcribe speech, sync with the web portal, or record in the background. The implemented transport components and remaining integration are documented in [mobile-transport.md](../../docs/a04/mobile-transport.md). This app is not evidence that a fabricated wearable is ready to use.

The current APK also contains the [durable download store](DOWNLOADS.md): validated transfer records and resume metadata commit together in SQLite, and a completed source can enter this same playable library. Its storage and decoding paths have passed Android runtime checks. The new [foreground recovery owner and Android GATT client](RECOVERY.md) connect that storage foundation to a serialized transport lifecycle. The separate [Zephyr radio DK service](../../firmware/a04/radio/README.md) is now implemented and requires [ASC1 owner proof](../../docs/a04/session-auth-v1.md). Android still needs that proof exchange, trusted owner-context custody, consumer enrollment and Activity integration before accessing the DK service. See the [verification record](VERIFICATION.md) for the executed scripted-link and virtual-radio scope; those earlier tests do not execute the Zephyr service, its proof handshake or physical pairing. The screenshots above show the earlier local-import UI, which this change does not replace.
