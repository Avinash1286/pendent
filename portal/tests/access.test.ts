...
const note = { title: 'Design review', transcript: 'Try a lighter chain.', summary: ['Try a lighter chain.'], actions: [], tags: ['design'], recordedAt: 100000, contextEnabled: false };
async function fixture() {
  const t = convexTest(schema, modules);
  const [aliceId, bobId] = await t.run(async ctx => [await ctx.db.insert('users', { name: 'Alice' }), await ctx.db.insert('users', { name: 'Bob' })]);
  return { t, aliceId, bobId, alice: t.withIdentity({ subject: `${aliceId}|sessionA` }), bob: t.withIdentity({ subject: `${bobId}|sessionB` }) };
}
...
test('ingest retries are idempotent and token permissions override caller context flags', async () => {
  ...
  const tokenId = await t.mutation(internal.tokens.insert, { ownerId: aliceId, name: 'Sync', hash: 'hash', prefix: 'aura_ingest_', scope: 'ingest', autoContext: false });
...
test('scope, revocation, expiration and owner boundaries are enforced for tokens', async () => {
  ...
  const id = await t.mutation(internal.tokens.insert, { ownerId: aliceId, name: 'Context', hash: 'secret-hash', prefix: 'aura_context_', scope: 'context', autoContext: false });
  expect(await t.query(internal.tokens.resolve, { hash: 'secret-hash', scope: 'ingest' })).toBeNull();
  ...
  expect(await t.query(internal.tokens.resolve, { hash: 'secret-hash', scope: 'context' })).toBeNull();
});

test('profile reads and updates are owner scoped and inputs are bounded', async () => {
  const { t, alice, bob } = await fixture();
  await alice.mutation(api.profiles.save, { name: 'Alice', about: 'Private goal', goals: '', preferences: '' });
  expect(await bob.query(api.profiles.get, {})).toBeNull();
  expect(await t.query(api.profiles.get, {})).toBeNull();
  await expect(alice.mutation(api.notes.save, { ...note, transcript: 'x'.repeat(60001) })).rejects.toThrow();
...
test('HTTP rejects missing credentials, wrong scope and malformed MCP payloads', async () => {
  ...
  await t.mutation(internal.tokens.insert, { ownerId: aliceId, name: 'MCP', hash, prefix: secret.slice(0, 20), scope: 'context', autoContext: false });
  ...
  const reply = await t.fetch('/mcp', { method: 'POST', headers, body: JSON.stringify({ jsonrpc: '2.0', id: 1, method: 'tools/list' }) });
...
test('context packs preserve source attribution and explicitly mark truncation', () => {
  ...
  expect(buildContext(null, Array.from({ length: 20 }, () => ({ ...note, transcript: 'x'.repeat(6000) })))).toContain('[Pack truncated');
});
