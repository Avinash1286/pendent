...
Firmware progress: [firmware/a04](../../firmware/a04/README.md) now connects a monitored PDM driver and bounded reader FIFO to one recorder/storage owner, unmodified libopus 1.6.1, the shared revision-3 archive, packed NAND journal and W25N01GV SPI adapter. [The audio contract](../../firmware/a04/AUDIO.md) defines driver-health faults, exact block/final-watermark checks, warmup, privacy ordering and DMA quarantine. Real 16 kHz mono, 32 kbps, 10/20 ms fixtures pass host and independent FFmpeg decode with exact duration.

Three separate DK targets have different evidence. The [synthetic integration
probe's current ELF report](../../firmware/a04/verification/arm-resources.json)
binds its exact memory, symbols and configuration; its RAM NAND and inactive
peripheral reservations are not a physical capture or Bluetooth workload. The
[wired bench](../../firmware/a04/bench/README.md) connects actual peripheral
drivers and UART export with Bluetooth disabled, with its own
[ARM resource report](../../firmware/a04/bench/verification/arm-resources.json).
The new [radio DK application](../../firmware/a04/radio/README.md) implements
the GATT adapter, local pairing policy and [ASC1 owner proof](session-auth-v1.md).
Its source and verification scope are independent of both earlier targets.
None has been flashed or executed on physical hardware. Final A04 pin mapping,
physical scheduling/audio/NAND/radio qualification, signed OTA and populated-block
reclamation remain required; a DK build is not the complete product memory budget.

Storage verification: **25 command/integration groups and 19 journal groups pass**, alongside the existing Python archive tests. [The command transcript](../../firmware/a04/verification/w25n-command-tests.txt) covers the actual command core, including two combined driver/journal cases; [journal roundtrip evidence](../../firmware/a04/verification/journal-roundtrip.json) binds real Opus → modeled power cut/reopen → AUR3 → durable Python/SQLite import and restart → independent FFmpeg decode. A finalized recording preserves 120847 source samples; staged/torn tails preserve 116440, and a completed page with lost acknowledgement recovers 120600. Interrupted original duration remains unknown. These are host-model results, not electrical NAND or assembled-device qualification.
...
Activity still exposes file import. The separate
[Zephyr radio DK adapter](../../firmware/a04/radio/README.md) is now implemented;
Android still needs its ASC1 proof exchange, trusted owner-context custody,
secure consumer enrollment and UI integration before accessing that target.
...
| Reliable reconnection | A04 bounded cursor, ordered C command owner and Kotlin reply/fragment parsers host-tested; Android recovery/GATT client and separate Zephyr radio DK adapter implemented with distinct verification boundaries | Integrate Android ASC1 proof, trusted owner-context custody, secure enrollment and consumer lifecycle/UI; qualify actual device-to-phone recovery and background behavior | iOS and Android foreground/background/process-death matrices, permissions off/on, bonds, out-of-range return |
...
these boundaries with scripted connections. The Android GATT client and separate
radio DK adapter are implemented; the Android ASC1 exchange, trusted owner-context
custody, secure enrollment, consumer UI integration, physical durability and
background lifecycle qualification remain separate work. See the
...
4. **Mobile loop:** durable Android source/receipt storage, exact prefix replay and decoded-library handoff are implemented and emulator-tested. The foreground recovery coordinator, Android GATT client and separate radio DK adapter have distinct source/test evidence. Integrate Android ASC1 proof, trusted owner-context custody, secure consumer enrollment and lifecycle/UI; verify the real microphone/NAND-to-phone path, then qualify actual OS background behavior and any acknowledged outbox. Timestamped transcript and bookmark navigation remain required. A timer is not proof of background execution. [Current recovery scope](../../mobile/android/RECOVERY.md), [execution evidence](../../mobile/android/VERIFICATION.md).
