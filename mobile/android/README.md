...
The committed [JVM transcript](core/verification/jvm-host.txt) records **10 groups / 231 checks**, using **17 C-generated fixtures** and rejecting an additional raw prefix. The [source-bound report](core/verification/kotlin-core.json) compares C archives and receipts, an independent Python parser/muxer, the Kotlin parser/muxer, exact Ogg bytes, and actual FFmpeg decoded sample counts. This is JVM/host evidence, not Android MediaCodec or physical microphone evidence.
