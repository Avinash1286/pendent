...
for n in names:
 ...
 if n.startswith(('08_','13_')):col=(.48,.51,.54)
 if n.startswith('14_'):col=(.55,.42,.22)
 ...
 if n.startswith(('08_','13_')):o.data.materials[0].node_tree.nodes['Principled BSDF'].inputs['Metallic'].default_value=.65
...
caption('Title','AURA / A04 M2 S1',-scale*.44,scale*.45,scale*.03)
...
caption('Limit','4.3 mm pack allocation; supplier pack unapproved.\n'+('12 polymer parts + 3 separate hardware forms + 2 unpowered gauges. Offsets illustrate parts, not assembly motion.' if a.view=='exploded' else 'Polymer champagne finish and light emission are visual intent; no powered fit release.'),-scale*.44,-scale*.433,scale*.01)
sc.render.engine='CYCLES';sc.cycles.samples=32;sc.cycles.use_denoising=False;sc.render.threads_mode='FIXED';sc.render.threads=4;sc.render.resolution_x=1400;sc.render.resolution_y=1400;sc.render.resolution_percentage=100;sc.render.image_settings.file_format='PNG';sc.render.image_settings.color_mode='RGB';sc.view_settings.view_transform='AgX';sc.render.filepath='//m2-'+a.view+'.png';bpy.context.preferences.filepaths.save_version=0
