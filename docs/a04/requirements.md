...
|---|---|---|
| Reference-aligned design | Inspected exterior/interior renders, dimensioned mechanical contract, actual component stack-up | Original 40 mm study plus separate 41 mm M1 mechanism trial at 10.8/11.8 mm depth; no native-placement or physical fit qualification |
...
| Printable, assembled body and dock | Watertight STL/3MF/STEP as applicable, units/wall/fit/clearance/fastener/motion checks, physical prints and assembled fit | Nine earlier fit coupons and two M1 sets of 14 STL specimens pass digital mesh/bounds checks. M1 has blocked straight face insertion, missing positive PCB support and full-press lightpipe interference; complete body/dock and physical fit remain |
| Firmware on actual hardware | Reproducible build, protocol/storage tests, real microphone/radio/charge/haptic/boot/OTA and fault tests | Real Opus/C archive, packed NAND journal and production SPI command core pass host tests: 20 command/integration groups, 11 journal groups and 33 receiver tests. ARM cross-build uses 194,356 B flash / 148,864 B RAM, including synthetic RAM NAND; no on-device execution or complete A04 firmware |
| Reliable local/mobile context loop | Real-phone capture/sync/recovery, durable receipts, source-linked transcription/search/context, account and permission tests | 95 companion tests and 24 portal tests pass; actual C archive → local Whisper → Python uploader → Next.js/Convex tested with synthetic speech. Physical transport/mobile/cloud integration remains |
