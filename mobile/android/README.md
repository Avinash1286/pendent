...
[Download the development APK](https://github.com/Avinash1286/pendent/releases/download/a04-android-local-dev/aura-a04-android-debug.apk) · [Release files and checksums](https://github.com/Avinash1286/pendent/releases/tag/a04-android-local-dev) · [Verification](VERIFICATION.md)
...
The current [JVM transcript](core/verification/jvm-host.txt) records **17 groups / 32,390 checks**: the existing 231 archive checks plus 32,159 response-fragment checks. It uses **17 C-generated fixtures** and rejects an additional raw prefix. The [source-bound report](core/verification/kotlin-core.json) compares C archives and receipts, an independent Python parser/muxer, the Kotlin parser/muxer, exact Ogg bytes, and actual FFmpeg decoded sample counts. The new fragment parser is an unused transport foundation; it adds no GATT or app download capability and is not in the earlier released APK. These are JVM/host checks, separate from the released APK's Android MediaCodec evidence.

**Android API 29 emulator execution passed: nine cases / 168 assertions.** The real `OMX.google.opus.decoder` produced 48 kHz mono PCM with exact retained durations; SQLite imports, edits, retries, receipt upgrades, invalid-source retention and cold-recovery cases passed. Both APKs build with zero Android lint findings. [Build report](verification/android-build.json) · [Runtime report](verification/android-runtime.json) · [Detailed verification and limits](VERIFICATION.md). This is emulator evidence with synthetic C fixtures, not a physical-phone or wearable test.
