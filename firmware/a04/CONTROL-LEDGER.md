...
Status: **implemented and tested against the strict host NAND model; no production erase integration or physical test**. This is a prerequisite for durable identity and owner-release state. It stores opaque bytes. It does not authenticate them, interpret a receipt, erase audio, evict a catalog entry, or implement daily-use reclamation.
...
There is intentionally no production implementation of this callback. The existing W25N driver's ordinary `aura_nand_io.erase` refuses populated blocks; it must remain that way. Supplying it as a fallback cannot safely implement rotation. Open/load work with a read-only backend, but provision/store require the separate callback. A future driver capability must permit only these reserved control blocks; an audio-reclamation capability would require a different reviewed authorization boundary.

## Security and remaining integration

The opaque snapshot must eventually contain atomically bound identity reservations, storage/owner generations, release replay floor, exact authenticated release envelope, and any fenced block extents. This module does not supply those semantics. A receipt alone is not deletion authority. The release controller must independently verify the authenticated owner command, exact local committed source, policy and scope before any release grant can authorize audio erasure.
...
The shared `scripts/build.ps1 -Mode all` now builds/runs this harness with `--build`, preserving [the source-bound report](verification/control-ledger.json), [host transcript](verification/control-host.txt) and synthetic control-pair fixture. It also cross-compiles the actual functions into the DK probe. [The ELF report](verification/arm-resources.json) measures the retained control context and functions. They are unprovisioned: no physical block pair or privileged erase callback is configured. The probe does not allocate a maximum-sized 49152-byte caller snapshot buffer or establish the complete product memory budget.
