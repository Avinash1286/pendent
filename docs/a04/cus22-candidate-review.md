...
The subsequent [72-reference assembly audit](assembly-attributes-review.md) identified a supported **placed-board** correction: `set_footprint_type` can set SMD classification and BOM/position exclusions after a footprint is placed. This does not edit the unassigned library definition or verify a future placement/export. Use that native operation and inspect the saved result when this candidate is actually adopted.
