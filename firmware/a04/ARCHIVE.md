...
The C writer uses no heap and keeps at most a 1301-byte pending record. Its object is 1504 bytes on the tested 64-bit host and 1496 bytes on ARM. The linked codec/archive/journal/SPI probe uses 194356 bytes flash and 148864 bytes RAM (including 16 KiB synthetic RAM NAND); [arm-resources.json](verification/arm-resources.json) records actual ELF symbols and hashes. SHA256 scratch is bounded, and compiler stack reports are lower bounds, not a runtime high-water measurement. The same ARM probe reserves a 32 KiB encoder arena and a 48 KiB usable codec thread stack, with guard/alignment overhead. These totals exclude the complete future PDM/NAND/BLE/OTA stack.
