...
The current checkpoint includes a **72-reference schematic candidate**, monitored microphone driver, bounded audio FIFO, recorder, shared C/Python Opus archive, recoverable NAND journal and SPI command core, plus separate tested authentication/control-snapshot primitives. The nRF52840 DK integration build records actual linked memory and symbols in [the ELF resource report](firmware/a04/verification/arm-resources.json). Host checks cover driver faults, privacy/stop ordering, exact capture boundaries and real-Opus recovery through simulated power cuts. The broader companion and portal checkpoints retain 95 and 24 passing tests respectively. The schematic still has one ERC error, nine warnings and three net-name mismatches; the board is not placed or routed. [Native capture evidence](docs/a04/native-capture-verification.json) · [Firmware, test counts and audio evidence](firmware/a04/README.md).
