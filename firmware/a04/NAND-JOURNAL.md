...
Header/checkpoint metadata occupy the first 256 bytes, with CRC32 at 252; bytes 256–2047 are FF when written. Both start with magic at 0, metadata version 1 or 2 at 4, zero at 5, size 256 at 6, and physical block number at 8. Legacy writing remains version 1 unless the caller explicitly enables an owned profile.

`A4NH`: previous block uint32 at 12 (`FFFFFFFF` for first), part uint32 at 16, starting archive byte offset uint64 at 20, device/capture IDs at 28, exact 68-byte AUR3 manifest at 60, 94-byte preceding committed-state declaration at 128 (all zero for first). Version 1 requires zero bytes 222–251. Version 2 stores a nonzero 16-byte storage incarnation at **222–237**, a nonzero uint64 allocation generation at **238–245**, and zeros at 246–251. The declaration is metadata, never an old-audio receipt.

`A4NC`: part uint32 at 12, valid data-page count uint16 at 16, zero at 18–19, committed archive byte end uint64 at 20, 94-byte page-end declaration at 28. Version 1 requires zeros at 122–251. Version 2 stores the same incarnation at **122–137**, the same allocation generation at **138–145**, and zeros at 146–251. It closes either a full block or a terminal short capture.
...
Version 2 changes only allocation metadata. `A4ND`, AUR3/AFR3/ASE3 and ACK3 remain unchanged. Every header and complete checkpoint of one capture must have the same metadata version, incarnation and allocation generation. Physical block, part and the exact header hash distinguish the individual extents of that allocation. Mixed-version chains, different namespaces/generations and noncanonical reserved bytes are corruption; independently identifying checkpoints still reconcile ownership even when allocation metadata is wrong.

## Owned allocation binding

After mount, a serialized owner can call `aura_journal_set_owned_profile(journal, incarnation)` to enable v2 writing. The namespace is nonzero and cannot change or be disabled until a fresh mount. Selecting the profile reads header/checkpoint prefixes of non-excluded blocks and raises a volatile generation floor from matching readable v2 metadata. It performs no audio scan. Unreadable metadata makes this conservative setup fail rather than inventing a floor.

Before `aura_recorder_start`, the owner calls `aura_journal_bind_capture(journal, exact_manifest68, generation)` with an already durably reserved nonzero generation. Only one binding can be pending. Its manifest is copied, and the next start must match all 68 bytes, not just device/capture IDs. A different valid manifest rejects and discards the binding. Preparation retries retain it; the successful start path consumes it **before the first header program attempt**, even if that attempt fails or loses completion. All continuation blocks then use the same capture generation. `aura_journal_discard_binding` cancels an unused idle binding without resetting the generation guard. Unbound owned starts and reused/lower generations are rejected before source programming.

The journal's volatile floor and observed metadata do not provide durable uniqueness after earlier allocations are erased or unreadable. The external storage owner must reserve generations durably, derive nonrepeating capture IDs, and preserve that reservation across reboot and catalog eviction. The journal does not authenticate the generation or derive IDs itself.

`aura_journal_get_allocation_identity` repeats full source and metadata verification before returning `version`, `owned`, `incarnation` and `allocation_generation`. Version 1 returns `owned=false` and zero allocation fields and is not eligible for an owned-release policy. For version 2, `owned=true` means identifiable allocation metadata only. It is **not deletion authority**, and can describe an open or interrupted prefix whose original duration remains unknown. Authenticated explicit owner release, exact local receipt, finalized-source policy and fenced erase decisions belong to the external controller. No identity result releases unassociated material.

The added state is one profile, one pending 68-byte manifest and scalar generations/flags. There are no added arrays per capture or block. The host context grows by **120 bytes**. The namespace and allocation generation are copied to local scratch during metadata walks, so mixed legacy/v2 inventories remain bounded.

## Mount, recovery and export

Mount validates bounds **1–1024 before indexing** and reads factory/LUT inventory through the initialized backend, plus header/checkpoint prefixes. It never scans all 128 MiB of source audio during startup. A completely factory-good 1024-block portable-model mount performs **2048 reads of 256 bytes = 524288 bytes, zero payload reads, zero programs and zero erases**. These portable counters exclude the adapter's separate raw-marker/LUT command scan. The test with 128 short captures on 129 blocks performs **386 metadata reads / 98816 bytes**, measured from the model, and zero payload reads. Device wall-clock startup is unmeasured.

Physical block order is independent of recording order. Mount collects predecessor claims, then rereads one 256-byte header per reachable owned block to establish each unique contiguous chain. Existing bounded owner/link tables suffice; no extra per-block RAM table or payload scan is added. Duplicate heads or successors, skipped/repeated parts, cycles, foreign predecessors and orphaned parts invalidate the affected captures. The allocator visits at most one full circuit of the block inventory to find a FREE candidate, including blocks below its cursor. This supports wraparound; it does **not** authorize reclamation of populated blocks.

The host regression uses the real writer to fill an unrelated block 2, then write a recording through **3 → 0 → 1**, obeying once-per-page and ascending writes within each block. Reopen performs 12 metadata reads / 3072 bytes across those four blocks; full verification/export preserves the exact 162813 archive bytes and receipt. Twelve deliberately damaged or conflicting chain layouts are refused with zero additional programs or erases. Catalog indices are discovery-local; persistent references must use device/capture IDs.

The corresponding mixed inventory with one v1 capture and a three-block v2 capture performs **18 metadata reads / 4608 bytes**, still with zero payload reads at mount. V2 header/checkpoint identity comparisons add bounded prefix reads. Explicit owned-profile selection adds two prefix reads per non-excluded block; identity retrieval later performs full verification on demand. Legacy-only mount counts above remain unchanged.
...
A valid checkpoint can associate a damaged identity header with a known capture; that capture is marked faulty and export fails. Checkpoint ownership is reconciled on every non-excluded block, including blocks whose headers appear valid. A continuation relabeled as a canonical head for another recording still faults both recordings when its checkpoint identifies the original one. Tests cover increasing and wrapped physical order. An otherwise valid checkpoint with an unknown identity faults its header owner and increments `unassociated_blocks`; unidentifiable candidates remain quarantined. An explicitly interrupted prefix does **not** prove that unassociated blocks contain no additional source audio. They are preserved for a future forensic/recovery tool; no normal finalized capture or deletion authority is inferred from them. This is a stated recovery limit of the bounded port.
...
The journal itself has **no populated-block reclamation**, including after host acknowledgement, and no catalog eviction or automatic format. Media-full stops recording and preserves prior audio; verified receipts and export remain available for synchronization. V2 allocation metadata is used by the separate [storage owner](STORAGE.md), which now tests durable authenticated grants, catalog reuse and retained-source preservation on host NAND. The metadata alone is not an erase permission. Production audio-erase integration, enrollment and recovery handling for unassociated material remain outstanding.
...
The current journal suite passes **19 groups**, including 13 v2 metadata/cross-owner corruption cases and **52 v2 program-cut cases** around header/checkpoint identity boundaries. It also verifies exact one-use binding, caller-manifest mutation, preparation retry, stale generation refusal, mixed legacy/v2 reading and identity revalidation after a previously verified checkpoint changes.
