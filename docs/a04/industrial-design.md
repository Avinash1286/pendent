...
The separate [M2 mechanism](../../enclosure/a04/mechanism-m2/README.md) replaces M1's blocked rear insertion with an open-front cup and removable threaded polymer bezel. Three positive PCB supports and matching retained clamp feet establish the actual 1.6 mm board plane. A guided captive face has independent 0.6 mm face and 0.25 mm calibrated shoe stops, a measured foam cartridge, full-area return-pad bosses and local radio relief. The completed face loads from the front. M1 and the original study below remain preserved historical checkpoints.

M2 is **43 × 12.2 mm** in the primary documented 4.3 mm pack-depth render, or **43 × 11.2 mm** for the unselected 3.3 mm pack goal. Both use Ø35.2 × 1.6 mm PCB gauges and a 26 × 21 mm pack at (−3.2, 0), with the 0.4 mm growth allowance modeled as a solid. Neither pack is approved; 0.8 mm PCB thickness remains an unauthored target. S1 revises the M2 files in place: 17 individual STL forms per scenario comprise 12 polymer parts, 3 separate hardware forms and 2 gauges. Metal and bonded-dielectric forms are not functional resin-print parts.

**S1 structural revision:** the former 0.30 mm resin shoe and 0.50 mm round resin arms are rejected. A separate 0.30 mm candidate stainless shoe with a measured insulating contact pad, and a separate 1.00 mm CAD maximum lower steel plate with 1.2 mm arms, replace those bending links. A wall-tied cup bed, carrier root cover and closure provide the reaction path outside the board and cell. Four stationary contacts enclose the actuator center and bound the nominal rigid shoe plane; this excludes flex, joint movement and contact-height error. Two independent keeper strips replace the obstructing U crossbar; each follows a sampled 3.5 mm slide and 5 mm rearward staging path and is positively blocked after face installation.

[Calculated screening](../../enclosure/a04/mechanism-m2/structural-screen.json) supports an instrumented unpowered joint specimen. The annealed-304 option fails the proposed 5 N one-arm metal screen: about 374 MPa after an assumed factor exceeds its 230 MPa reference. The adopted candidate is ASTM A666 Type 301 half-hard with explicitly certified 0.2% yield at least 110 ksi / 758 MPa; the same factored screen has a ratio of about 2.03. Supplier thickness capability is not confirmed stock or a qualified cutting process. E = 193 GPa predicts slightly more deflection: unequal-load support surrogates reach about 0.060 mm at 1.5 N and 0.199 mm at 5 N before joint compliance. The worst screened root hold-down is about 53.3 N with 23.1 MPa nominal lower bearing. Printed root, liner, clamp and actual endpoint qualification remain open.

A positive rear guide tongue and separate keeper edge slots close the nominal rear capture path. Its roof-limited pitch section has about 0.232 mm retention margin, excluding dimensional/coplanarity error and full 3D tilt/yaw behavior. A 0.50 mm shoe in the 0.51 mm nominal stack allowance is explicitly rejected. The thinner steel leaves a nominal 0.19–0.23 mm measured insulating-pad allocation before printed and mounted-height uncertainty; it is not a worst-case tolerance approval.

Both S1 digital scenarios pass the scoped checks: 57 rigid and 13 working soft allocations, 25 coupled capture poses, three privacy cases with 54 take-up samples each, and 17 insertion/subassembly/tool paths totaling 679 samples. These are finite CSG samples, not continuous motion, force or tolerance proof. All 72 native PCB features / 68 purchased PCB references remain pending; native board component count is zero at this checkpoint. Most package geometry, RF, supplier pack, physical prints, root-joint strength, actuator endpoints, material creep, acoustic/thermal and worn-device qualification remain release gates. M2 is **not a print or wearable release**.
