...
const sourceFiles = [
  'components/aura-experience.tsx',
  'components/device-viewer.tsx',
  'app/globals.css',
  'index.html',
];
...
for (const file of sourceFiles) {
  ...
  for (const match of source.matchAll(
    /["'(](\/(?:product|film|fonts)\/[^"')]+|\/favicon\.svg)["')]/g,
  ))
...
console.log(
  JSON.stringify(
    {
      status: 'passed',
      localAssetCount: references.size,
      assets: [...references].map((reference) => ({
        path: reference,
        bytes: fs.statSync(new URL(`public${reference}`, base)).size,
      })),
    },
    null,
    2,
