...
The [native Android companion](mobile/android/README.md) adds a local recording library: select an A04 archive, validate its original bytes and Opus audio, listen back, write context, and explicitly copy/share that context with an AI chat. It preserves interrupted-capture provenance and the original recording. The published local-import APK passed nine Android emulator cases / 168 assertions and built with zero lint findings. The current Kotlin core separately adds response-fragment parsing and an exact transfer codec, checked against actual C-generated replies: 24 JVM groups / 32,728 checks pass. These transport components are not yet connected to GATT or included in the earlier APK. [Verification](mobile/android/VERIFICATION.md).

A [bounded journal cursor](firmware/a04/JOURNAL-CURSOR.md) prepares pendant-to-phone recovery: exact-record resume, at most one NAND read per step, source checks before streaming and again before completion, and cancellation that preserves the original recording. It is exercised through the wired DK export path. A new portable [transfer command owner](docs/a04/transfer-wire-v1.md) adds ordered retries, owned capture selection and protection against stale response callbacks; 12 C test groups pass. The Android GATT connection, ownership enrollment and durable download coordinator remain outstanding. [Transport integration plan](docs/a04/mobile-transport.md).
