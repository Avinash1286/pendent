...
**Current first-unit route:** [AURA: from Omi's lessons to a first wearable](aura-first-wearable.md) tracks the fresh circular A04, the actual microphone/NAND bench milestone, phone recovery, fabrication gates and personal wear acceptance. Use it for present build sequencing. The A03 geometry findings and earlier implementation checkpoints below retain their historical scope.

**A04 implementation update:** the baseline findings below are retained as reviewed. Since that review, AURA has added monitored PDM ingress, a bounded audio FIFO, serialized recorder, real Opus encoding, a recoverable NAND journal/SPI command core and the shared C/Python revision-3 archive. The storage owner now reserves durable capture identities and tests exact authenticated release/reuse through the committed Python receiver and release outbox, preserving another recording byte-for-byte. Its populated-audio erase still uses modeled NAND. The DK cross-build and host fault tests pass; no A04 physical recording has been qualified. Local import, real Whisper transcription, stable owner/capture ingestion across token rotation and retained portal source metadata also exist. The current companion suite passes 131 tests; actual local Next.js/Convex integration used synthetic speech. [Current firmware evidence](../../firmware/a04/README.md), [companion evidence](../../companion/VERIFICATION.md) and [the A04 requirements matrix](../a04/requirements.md) distinguish that progress from outstanding device, mobile, electrical and mechanical work.

The [Android recovery owner and GATT client](../../mobile/android/RECOVERY.md) now connect durable phone storage to the playable library. Their [Android and virtual-peripheral verification](../../mobile/android/VERIFICATION.md) is separate from the new [radio DK adapter, owner-proof provider and pairing policy](../../firmware/a04/radio/README.md). The Android ASC1 exchange, trusted enrollment, consumer UI integration and physical hardware-to-phone recovery remain unfinished; the earlier companion tests do not cover these newer components.

**Decision:** keep AURA's deliberate capture, physical microphone disconnect, recoverable local audio and owner-approved AI context. Adopt Omi's attention to reconnection, compression, mobile recovery and measured hardware tests. First validate recording on an off-the-shelf development board, then complete the electrical, enclosure and power checks before wearing it. Revise AURA's component placement before ordering assembled custom boards. Omi's consumer hardware and AURA's design require their own matched fabrication and assembly review.
...
| Phone | [`app/`](https://github.com/BasedHardware/omi/tree/f42089f53c8010dd971b3702ece05a231c9c0c66/app) | Flutter iOS/Android capture, sync, pending uploads and result presentation. This was the largest missing everyday-use layer at the reviewed AURA baseline; the current Android foundation and remaining integration are linked above. |
...
Improvement: retain an identified frame in durable storage until the receiving application commits it, replay gaps after reconnection, and expose an explicit overflow/failure state. Test connection-before-subscription, reconnect during a sentence, notification exhaustion and phone process termination. AURA's local-recording-before-sync design follows this direction; its physical reliability still needs measurement.

### P1: radio transmit completion is weaker than durable app acknowledgement

Consumer storage counts bytes in `storage_data_tx_done()` and uses that count to advance the persisted SD read pointer. The callback is attached to a GATT notification. Its comments describe the phone as having confirmed receipt, but Zephyr's callback indicates transmission, not completion of an application file/database write. Advancement removes data from the unread set; it does not itself physically erase those SD bytes. A phone crash before durable publication can therefore leave no durable phone copy and no unread device entry for ordinary recovery. Later ring writes can overwrite the remaining device bytes. [Storage callback](https://github.com/BasedHardware/omi/blob/f42089f53c8010dd971b3702ece05a231c9c0c66/omi/firmware/omi/src/lib/core/storage.c#L245-L255), [pointer advancement](https://github.com/BasedHardware/omi/blob/f42089f53c8010dd971b3702ece05a231c9c0c66/omi/firmware/omi/src/lib/core/storage.c#L384-L435), [ring overwrite handling](https://github.com/BasedHardware/omi/blob/f42089f53c8010dd971b3702ece05a231c9c0c66/omi/firmware/omi/src/sd_card.c#L513-L532), [Zephyr GATT semantics](https://docs.zephyrproject.org/latest/services/connectivity/bluetooth/api/gatt.html).
...
Improvement: acknowledge `{device, stream, committed sequence, checksum}` from the app after durable local publication; make acknowledgements monotonic and idempotent. Test killing the receiver after notification but before its file commit. The reviewed AURA baseline requires an explicit deletion step after CRC verification; radio completion must not replace application verification and authorized release. No relative transfer or deletion speed was measured.
...
## Historical companion follow-up at the reviewed baseline
...
See [companion verification](../../companion/VERIFICATION.md) for this checkpoint's executed tests and later companion evidence. This initial follow-up did not change the firmware, PCB, case or release archive. Mobile recovery and authenticated device identity were then outstanding; the Android/radio progress linked above has since addressed parts of that work. Consumer enrollment and UI integration, physical recovery, a durable upload outbox, measured transfer performance and a qualified charger/dock remain separate requirements.
